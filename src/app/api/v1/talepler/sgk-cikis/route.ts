...
export const POST = createAuthorizedRouteHandler("ik_5", "create", async (request: NextRequest, session) => {
  const payload = await request.json();
  const talep = await createTalep(session.tenantId, "sgk-cikis", payload);
