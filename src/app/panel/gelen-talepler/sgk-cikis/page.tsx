...
export default function Page() {
  ...
  const [activeModule, setActiveModule] = useState<keyof typeof MODULE_CONFIGS>("sgk-cikis");
  ...
  const handleAction = async (id: string, newStatus: string) => {
    ...
    setTalepler((prev) => prev.map((item) => (item.id === id ? { ...item, durum: newStatus } : item)));
    // SGK giriş / çıkış onayında sicil güncellemesi sunucuda yapılır (updateTalepStatus).
  };
              ...
              {item.durum !== 'BEKLEYEN' && (
                <div className="w-full flex justify-between items-center text-[9px] font-extrabold px-1">
                  ...
                  <div className="flex flex-col gap-1 text-right">
                    <span className="text-[#172b4d]">23.03.2026</span>
                    <span className="text-[#172b4d]">kargaliogluemre@gmail.com</span>
                  </div>
                </div>
              )}
