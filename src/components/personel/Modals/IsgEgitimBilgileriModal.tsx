...
export default function IsgEgitimBilgileriModal({ isOpen, onClose, data, onSave }: Props) {
  const [draft, setDraft] = useState<Partial<Personel>>({});

  useEffect(() => {
    if (isOpen) {
      setDraft({
        isgEgitimDurumu: data.isgEgitimDurumu || "",
        isgEgitimTuru: data.isgEgitimTuru || "",
        isgEgitimSuresi: data.isgEgitimSuresi || "",
        isgEgitimiVeren: data.isgEgitimiVeren || "",
      });
    }
  }, [isOpen, data]);
    ...
    <div className="fixed inset-0 bg-[#091e42]/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b-2 border-gray-50 shrink-0">
          <h2 className="text-[16px] font-black text-[#172b4d]">İSG Eğitim Bilgileri</h2>
        ...
        <div className="p-6 overflow-y-auto flex flex-col gap-4">
          <SelectField label="Eğitim Durumu" value={draft.isgEgitimDurumu || ""} onChange={set("isgEgitimDurumu")} options={["Aldı", "Almadı"]} />
          <TextField label="Eğitim Türü" value={draft.isgEgitimTuru || ""} onChange={set("isgEgitimTuru")} />
          <TextField label="Eğitim Süresi" value={draft.isgEgitimSuresi || ""} onChange={set("isgEgitimSuresi")} placeholder="Örn: 16 Saat" />
          <TextField label="Eğitimi Veren" value={draft.isgEgitimiVeren || ""} onChange={set("isgEgitimiVeren")} />
        </div>
