...
export default function MykMeslekiYeterlilikModal({ isOpen, onClose, data, onSave }: Props) {
  const [draft, setDraft] = useState<Partial<Personel>>({});

  useEffect(() => {
    if (isOpen) {
      setDraft({
        mykBelgesi: data.mykBelgesi || "",
        meslekAdi: data.meslekAdi || "",
        mykSeviye: data.mykSeviye || "",
        mykBelgeNo: data.mykBelgeNo || "",
      });
    }
  }, [isOpen, data]);
    ...
    <div className="fixed inset-0 bg-[#091e42]/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b-2 border-gray-50 shrink-0">
          <h2 className="text-[16px] font-black text-[#172b4d]">MYK Mesleki Yeterlilik</h2>
        ...
        <div className="p-6 overflow-y-auto flex flex-col gap-4">
          <SelectField label="MYK Belgesi" value={draft.mykBelgesi || ""} onChange={set("mykBelgesi")} options={["Var", "Yok"]} />
          <TextField label="Meslek Adı" value={draft.meslekAdi || ""} onChange={set("meslekAdi")} />
          <TextField label="Seviye" value={draft.mykSeviye || ""} onChange={set("mykSeviye")} />
          <TextField label="Belge No" value={draft.mykBelgeNo || ""} onChange={set("mykBelgeNo")} />
        </div>
