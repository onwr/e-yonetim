...
export default function AskerlikDurumuModal({ isOpen, onClose, data, onSave }: Props) {
  const [draft, setDraft] = useState<Partial<Personel>>({});

  useEffect(() => {
    if (isOpen) {
      setDraft({
        askerlikDurumu: data.askerlikDurumu || "",
        terhisTarihi: data.terhisTarihi || "",
        hizmetTuru: data.hizmetTuru || "",
        sinifBrans: data.sinifBrans || "",
      });
    }
  }, [isOpen, data]);
    ...
    <div className="fixed inset-0 bg-[#091e42]/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b-2 border-gray-50 shrink-0">
          <h2 className="text-[16px] font-black text-[#172b4d]">Askerlik Durumu</h2>
        ...
        <div className="p-6 overflow-y-auto flex flex-col gap-4">
          <SelectField label="Askerlik Durumu" value={draft.askerlikDurumu || ""} onChange={set("askerlikDurumu")} options={["Yapıldı", "Tecilli", "Muaf", "Yapılmadı"]} />
          <DateField label="Terhis Tarihi" value={draft.terhisTarihi || ""} onChange={set("terhisTarihi")} placeholder="gg.aa.yyyy" />
          <TextField label="Hizmet Türü" value={draft.hizmetTuru || ""} onChange={set("hizmetTuru")} />
          <TextField label="Sınıf / Branş" value={draft.sinifBrans || ""} onChange={set("sinifBrans")} />
        </div>
