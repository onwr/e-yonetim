...
export default function EgitimVeMeslekiBilgilerModal({ isOpen, onClose, data, onSave }: Props) {
  ...
  useEffect(() => {
    if (isOpen) {
      setDraft({
        ...
        bolum: data.bolum || "",
        mezuniyetYili: data.mezuniyetYili || "",
      });
    }
  }, [isOpen, data]);
    ...
    <div className="fixed inset-0 bg-[#091e42]/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b-2 border-gray-50 shrink-0">
          <h2 className="text-[16px] font-black text-[#172b4d]">Eğitim ve Mesleki Bilgiler</h2>
        ...
        <div className="p-6 overflow-y-auto flex flex-col gap-4">
          <SelectField label="Eğitim Durumu" value={draft.egitimDurumu || ""} onChange={set("egitimDurumu")} options={["İlkokul", "Ortaokul", "Lise", "Önlisans", "Lisans", "Yüksek Lisans", "Doktora"]} />
          <TextField label="Okul Adı" value={draft.okulAdi || ""} onChange={set("okulAdi")} />
          <TextField label="Bölüm" value={draft.bolum || ""} onChange={set("bolum")} />
          <TextField label="Mezuniyet Yılı" value={draft.mezuniyetYili || ""} onChange={set("mezuniyetYili")} type="number" />
        </div>
