...
export default function IseGirisSaglikMuayenesiModal({ isOpen, onClose, data, onSave }: Props) {
  const [draft, setDraft] = useState<Partial<Personel>>({});

  useEffect(() => {
    if (isOpen) {
      setDraft({
        isGirisCalisabilirlik: data.isGirisCalisabilirlik || "",
        isGirisSaglikRaporu: data.isGirisSaglikRaporu || "",
        isGirisRaporTarihi: data.isGirisRaporTarihi || "",
        isGirisSonrakiMuayene: data.isGirisSonrakiMuayene || "",
      });
    }
  }, [isOpen, data]);
    ...
    <div className="fixed inset-0 bg-[#091e42]/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b-2 border-gray-50 shrink-0">
          <h2 className="text-[16px] font-black text-[#172b4d]">İşe Giriş Sağlık Muayenesi</h2>
        ...
        <div className="p-6 overflow-y-auto flex flex-col gap-4">
          <SelectField label="Çalışabilirlik" value={draft.isGirisCalisabilirlik || ""} onChange={set("isGirisCalisabilirlik")} options={["Uygundur", "Uygun Değildir"]} />
          <SelectField label="Sağlık Raporu" value={draft.isGirisSaglikRaporu || ""} onChange={set("isGirisSaglikRaporu")} options={["Var", "Yok"]} />
          <DateField label="Rapor Tarihi" value={draft.isGirisRaporTarihi || ""} onChange={set("isGirisRaporTarihi")} placeholder="gg.aa.yyyy" />
          <DateField label="Sonraki Muayene" value={draft.isGirisSonrakiMuayene || ""} onChange={set("isGirisSonrakiMuayene")} placeholder="gg.aa.yyyy" />
        </div>
