...
const TextAreaField = ({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) => (
  <div className="flex flex-col gap-1.5">
    ...
    <textarea
      ...
      onChange={(e) => onChange(e.target.value)}
      rows={3}
      className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-100 hover:border-gray-200 focus:border-[#0052cc] focus:ring-4 focus:ring-[#0052cc]/10 outline-none text-[13.5px] font-medium text-[#172b4d] transition-all resize-none"
    />
  </div>
);

export default function YasalDurumVeAdliSicilModal({ isOpen, onClose, data, onSave }: Props) {
  const [draft, setDraft] = useState<Partial<Personel>>({});

  useEffect(() => {
    if (isOpen) {
      setDraft({
        adliSicilKaydi: data.adliSicilKaydi || "",
        adliSicilNedeni: data.adliSicilNedeni || "",
        hacizAlimi: data.hacizAlimi || "",
        eskiHukumlu: data.eskiHukumlu || "",
        cezaNedeni: data.cezaNedeni || "",
        cezaNedeniDetayi: data.cezaNedeniDetayi || "",
        cocukOrnegi: data.cocukOrnegi || "",
        nafakaDurumu: data.nafakaDurumu || "",
        icraDurumu: data.icraDurumu || "",
        denetimliSerbestlik: data.denetimliSerbestlik || "",
        ozelNeden: data.ozelNeden || "",
      });
    }
  }, [isOpen, data]);
    ...
    <div className="fixed inset-0 bg-[#091e42]/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b-2 border-gray-50 shrink-0">
          <h2 className="text-[16px] font-black text-[#172b4d]">Yasal Durum ve Adli Sicil</h2>
        ...
        <div className="p-6 overflow-y-auto flex flex-col gap-4">
          <SelectField label="Adli Sicil Kaydı" value={draft.adliSicilKaydi || ""} onChange={set("adliSicilKaydi")} options={["Var", "Yok"]} />
          <TextAreaField label="Adli Sicil Nedeni" value={draft.adliSicilNedeni || ""} onChange={set("adliSicilNedeni")} />
          <SelectField label="Haciz Alımı" value={draft.hacizAlimi || ""} onChange={set("hacizAlimi")} options={["Var", "Yok"]} />
          <SelectField label="Eski Hükümlü" value={draft.eskiHukumlu || ""} onChange={set("eskiHukumlu")} options={["Var", "Yok"]} />
          <TextField label="Ceza Nedeni" value={draft.cezaNedeni || ""} onChange={set("cezaNedeni")} />
          <TextAreaField label="Ceza Nedeni Detayı" value={draft.cezaNedeniDetayi || ""} onChange={set("cezaNedeniDetayi")} />
          <SelectField label="Çocuk Örneği" value={draft.cocukOrnegi || ""} onChange={set("cocukOrnegi")} options={["Var", "Yok"]} />
          <SelectField label="Nafaka Durumu" value={draft.nafakaDurumu || ""} onChange={set("nafakaDurumu")} options={["Var", "Yok"]} />
          <SelectField label="İcra Durumu" value={draft.icraDurumu || ""} onChange={set("icraDurumu")} options={["Var", "Yok"]} />
          <SelectField label="Denetimli Serbestlik" value={draft.denetimliSerbestlik || ""} onChange={set("denetimliSerbestlik")} options={["Var", "Yok"]} />
          <TextField label="Özel Neden" value={draft.ozelNeden || ""} onChange={set("ozelNeden")} />
        </div>
