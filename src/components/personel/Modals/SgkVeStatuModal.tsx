  ...
  <div className="flex flex-col gap-1.5">
    ...
    <div className="relative">
      <select
        ...
        className={`w-full pl-4 pr-10 py-2.5 rounded-xl border-2 ${borderColor} hover:border-gray-200 focus:border-[#ef5a28] outline-none text-[13.5px] font-medium text-[#172b4d] appearance-none focus:ring-4 focus:ring-[#ef5a28]/10 transition-all cursor-pointer`}
      >
...
export function SgkVeStatuModal({ isOpen, onClose, data, onSave }: Props) {
  const [draft, setDraft] = useState<Partial<Personel>>({});

  useEffect(() => {
    if (isOpen) {
      setDraft({
        sgkStatusu: data.sgkStatusu || "",
        istihdamTuru: data.istihdamTuru || "",
        calismaTuru: data.calismaTuru || "",
        iseBaslamaTarihi: data.iseBaslamaTarihi || "",
        istenAyrilisTarihi: data.istenAyrilisTarihi || "",
      });
    }
  }, [isOpen, data]);
    ...
    <div className="fixed inset-0 bg-[#091e42]/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b-2 border-gray-50 shrink-0">
          <h2 className="text-[16px] font-black text-[#172b4d]">SGK & Statü</h2>
        ...
        <div className="p-6 overflow-y-auto flex flex-col gap-4">
          <SelectField label="SGK Statüsü" value={draft.sgkStatusu || ""} onChange={set("sgkStatusu")} options={["Normal Çalışan", "Emekli"]} borderColor="border-[#ef5a28]" />
          <SelectField label="İstihdam Türü" value={draft.istihdamTuru || ""} onChange={set("istihdamTuru")} options={["Tam Zamanlı", "Yarı Zamanlı", "Sözleşmeli"]} borderColor="border-[#ef5a28]" />
          <TextField label="Çalışma Türü / Model" value={draft.calismaTuru || ""} onChange={set("calismaTuru")} />
          <DateField label="İşe Başlama Tarihi" value={draft.iseBaslamaTarihi || ""} onChange={set("iseBaslamaTarihi")} placeholder="gg.aa.yyyy" />
          <DateField label="İşten Ayrılış Tarihi" value={draft.istenAyrilisTarihi || ""} onChange={set("istenAyrilisTarihi")} placeholder="Devam Ediyor" />
        </div>
